...

{
    ...
    //     // $period = collect($period->get())->toArray();
    //     // dd($period);
    ...
    public function get_attend_absence(Request $request)
    {

        // dd($request->all());
        // DB::enableQueryLog();
        $period = Attendance::with('attendance_details');

        if ($request->date) {

            $r = 0;
            $period = $period->whereHas('attendance_details', function ($query) use ($request) {
            ...
            if ($r == 0) {
                ...
                $period = $period->where('attendances.attendance_date', '=', $request->date);
            }
        }
        ...
        if ($request->work_system_id) {
            ...
            $period = $period->whereHas('staff.work_system_type', function ($query) use ($request) {

                            ...
                            <div v-for="(accounts, index) in list_data" :key="index" class="row"
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="custom-search" v-if="accounts.account_id">
                                        <input style="background-color: beige;" :id="'Sale_' + accounts.code + '_tree'"
                                            type="text" readonly class="custom-search-input"
                                            :value="accounts.account_id + accounts.account.first_name">
                                        <input style="background-color: beige;" :id="'Sale_' + accounts.code + '_tree_id'"
                                            type="hidden" readonly class="custom-search-input" :value="accounts.account_id">
                                        <input style="background-color: beige;"
                                            :id="'Sale_' + accounts.code + '_tree_hraccount_id'" type="hidden" readonly
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="custom-search" v-if="accounts.account_second_id">
                                        <input style="background-color: beige;"
                                            :id="'Sale_' + accounts.code + accounts.type + '_tree'" type="text" readonly
                                            class="custom-search-input"
                                            :value="accounts.account_second_id + accounts.account_second.second_name">
                                        <input style="background-color: beige;"
                                            :id="'Sale_' + accounts.code + accounts.type + '_tree_id'" type="hidden"
                                            readonly class="custom-search-input" :value="accounts.account_second_id">
                                        <input style="background-color: beige;"
                                            :id="'Sale_' + accounts.code + accounts.type + '_tree_hraccount_id'"
    ...
    methods: {
        ...
        submitForm() {
            ...
            for (let index = 0; index < this.count_accounts; index++) {
                ...
                formData.append(`sale_${this.list_data[index].code}_account_id`,
                    $(`#Sale_${this.list_data[index].code}_tree_id`).val()

                );
                formData.append(`sale_${this.list_data[index].code}_account_hraccount_id`,
                    $(`#Sale_${this.list_data[index].code}_tree_hraccount_id`).val(),

                );
                formData.append(`sale_${this.list_data[index].code}_account_name`, [
                    $(`#Sale_${this.list_data[index].code}_tree`).val(),

                ]);

                // ----------------
                formData.append(`sale_${this.list_data[index].code + this.list_data[index].type}_second_account_id`,
                    $(`#Sale_${this.list_data[index].code + this.list_data[index].type}_tree_id`).val()
                );
                formData.append(`sale_${this.list_data[index].code + this.list_data[index].type}_second_account_hraccount_id`,
                    $(`#Sale_${this.list_data[index].code + this.list_data[index].type}_tree_hraccount_id`).val()

                );
                formData.append(`sale_${this.list_data[index].code + this.list_data[index].type}_second_account_name`,
                    $(`#Sale_${this.list_data[index].code + this.list_data[index].type}_tree`).val()
            ...
            // send upload request
            this.axios
                .post("/store_sale_account_setting", formData, config)
